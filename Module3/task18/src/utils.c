...
// - Убрали бесконечный цикл
// - Сделали возвращение статуса обработки данных, вместо завершения форкнутого процесса
int dostuff(int socket) {
    ...
    if (len < 0) return -1;
    if (write(socket, buff, len) < 0) return -1;
