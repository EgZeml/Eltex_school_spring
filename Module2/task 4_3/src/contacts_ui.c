...
Contact add_contact_ui() {
    ...
    flush_input();
    if (answer_tmp == 'y' || answer_tmp == 'Y') {
        printf("Введите количесво записей: ");
        scanf(" %d", &number_tmp);
        flush_input();
...
Contact edit_contact_ui(Tree_node* node) {
    Contact updated = node->data;
    Contact c = node->data;
    char answer;
...
void inorder(Tree_node* root) {
    if (root != NULL) {
        inorder(root->left);
        display_contact(&root->data);
        inorder(root->right);
    }
}

// Функция для печати дерева с отступами, показывающая структуру дерева по id.
// 'indent' – текущий уровень отступа (например, 0 при вызове с корневым узлом).
void print_tree(Tree_node* root, int indent) {
    if (root == NULL)
        return;

    // Сначала печатаем правое поддерево, увеличивая отступ
    ...
    // Печатаем id текущего узла
    printf("%d\n", root->data.id);

    // Затем печатаем левое поддерево, также увеличивая отступ
