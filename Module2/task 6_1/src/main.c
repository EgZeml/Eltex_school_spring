    ...
    do {
        ...
        switch (choice) {
            ...
            case 2:
                ...
                id = read_int();
                node = find_contact_index(&book, id);
                if (node == NULL) {
                    printf(COLOR_RED "Ошибка: контакт не найден.\n" COLOR_RESET);
                    break;
                }
                contact_tmp = edit_contact_ui(&book, node);
                if (edit_contact_logic(&book, node, &contact_tmp) == 0)
            ...
            case 3:
                ...
                id = read_int();
                node = find_contact_index(&book, id);
                if (node != NULL && delete_contact_logic(&book, node) == 0)
